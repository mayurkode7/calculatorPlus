...
export default function BasicCalculator() {
  ...
  const [formatOption, setFormatOption] = useState("Indian");
  ...
  const getButtonClasses = (button) => {
    const base =
      "flex items-center justify-center rounded-full text-2xl h-16 sm:h-20 hover:brightness-110 active:brightness-90 select-none";
    const tone =
  ...
  const handleOperator = (symbol) => {
    ...
    if (lastActionWasEquals) {
      ...
      setExpressionParts((prev) =>
        prev.length > 0 ? [...prev.slice(0, -1), symbol] : prev
      );
  ...
  const liveExpression = lastActionWasEquals
    ...
    ? expressionParts.join(" ") +
      (!waitingForNewValue && pendingOperator ? " " + displayValue : "")
    : "";
  ...
  // Helper to format number in Indian system
      ...
      <div className="flex items-center justify-between md:hidden mb-2">
        <div className="flex items-center">
          ...
          <select
            ...
            className="ml-2 text-xs px-2 py-1 rounded border border-gray-300 bg-white text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      <main className="flex-1 w-full flex flex-col md:flex-row gap-6 md:gap-8">
        <section className="flex-1 flex items-center justify-center">
          <div className="w-full max-w-sm sm:max-w-md md:max-w-lg bg-black rounded-3xl p-4">
            <div className="text-right text-gray-400 text-sm pr-2 h-5 truncate">
              {liveExpression}
            </div>
            <div
              className={`text-right text-white ${getDisplaySizeClass(
                displayValue
              )} font-light leading-none py-6 pr-2 whitespace-nowrap overflow-hidden`}
            >
              {formatOption === "Indian"
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          ...
                          onClick={(e) =>
                            ...
                            Visit: https://calculator-plus.vercel.app/basic-calculator
                            `,
                              e.currentTarget,
                              index
                            )
